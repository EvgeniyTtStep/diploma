...
import com.example.diploma.email.events.registration.OnRegistrationCompleteEvent;
...
import com.example.diploma.model.schedulegrid.*;
...
import com.example.diploma.repositories.*;
...
import com.example.diploma.repositories.users.StudentRepository;
import com.example.diploma.repositories.users.TeacherRepository;
import com.example.diploma.repositories.users.UserRepository;
import com.example.diploma.security.DataValidator;
import com.example.diploma.security.service.AppUserDetailsService;
...
public class AdminPagesController {
    ...
    //Контролер для відображення головної сторінки адміністратора
    ...
    // Контролер для відображення списку викладачів
    ...
    // Контролер для видалення викладач за його ідентифікатором
    ...
    //Контролер для відображення списку студентів
    ...
    public String students(@PathVariable Long uid, Model model, HttpServletRequest request) {
        ...
        if (checkAuthResult.equals("**success**")) {
            ...
            return "new_students_manage";
        } else {
            return checkAuthResult;
        }
    }

    //Контролер для перегляду інформації про студента в групі
    @GetMapping("logged-in/{uid}/group/{gid}/members/student/{sid}/student_info")
    public String student_group_info(@PathVariable Long uid,
    ...
    //  Контролер для перегляду загальної інформації про студента
    @GetMapping("logged-in/{uid}/members/student/{sid}/student_info")
    public String student_info(@PathVariable Long uid,
                               @PathVariable Long sid,
                               Model model,
                               HttpServletRequest request) {
        String checkAuthResult = checkAuth(uid, request);
    ...
    // Контролер для видалення студента з групи
    ...
    // Контролер для видалення студента з університету
    ...
    //Контролер для перегляду списку семестрів
    ...
    public String teacher_invite(@PathVariable Long uid, Model model, HttpServletRequest request) {
        String checkAuthResult = checkAuth(uid, request);
        model.addAttribute("school", userRepo.findById(uid).get().getSchool());
        if (checkAuthResult.equals("**success**")) {
            return "new_registration_teacher";
        } else {
            return checkAuthResult;
        }
    }
    ...
    public String create_semester(@PathVariable Long uid, Model model, HttpServletRequest request) {
        String checkAuthResult = checkAuth(uid, request);
        model.addAttribute("school", userRepo.findById(uid).get().getSchool());

        if (checkAuthResult.equals("**success**")) {
            return "new_create_semester";
        } else {
            return checkAuthResult;
        }
    }
    ...
    public String schedule_edit(@PathVariable Long uid,
                                @PathVariable Long sid,
                                @PathVariable String for_a,
                                @PathVariable Long for_id,
                                Model model,
                                HttpServletRequest request) {
...
//    }
    ...
    @PostMapping("logged-in/{uid}/semester/{sid}/edit_timeslots")
    @ResponseBody
    public RedirectView config_timeslots(@PathVariable Long uid,
                                         @PathVariable Long sid,
                                         @RequestParam String[] timeslotname,
                                         @RequestParam String[] time_start,
                                         @RequestParam String[] time_end,
        ...
        // 1️⃣ Проверка: У всех ли массивов одинаковая длина?
        if (timeslotname.length != time_start.length || timeslotname.length != time_end.length) {
            throw new IllegalArgumentException("Input arrays must have the same length.");
        }

        // 2️⃣ Проверка корректности времени (формат и логическая валидность)
        for (int i = 0; i < timeslotname.length; i++) {
            int[] start = parseTime(time_start[i]);
            int[] end = parseTime(time_end[i]);

            // Проверка: время начала >= времени окончания
            if (convertToMinutes(start[0], start[1]) >= convertToMinutes(end[0], end[1])) {
                return new RedirectView("/err");
            }

            // Проверка: Нет ли перекрытия с последующим временным слотом
            if (i < timeslotname.length - 1 && isTimeOverlapping(time_start[i], time_end[i], time_start[i + 1], time_end[i + 1])) {
                return new RedirectView("/err");
            }
        }

        // 3️⃣ Проверка авторизации пользователя
        String checkAuthResult = checkAuth(uid, request);
        if (!AUTH_SUCCESS.equals(checkAuthResult)) {
            return new RedirectView("/" + checkAuthResult);
        }

        // 4️⃣ Поиск семестра и обновление временных слотов
        Optional<Semester> op_semester = semesterRepo.findById(sid);
        if (op_semester.isPresent() && op_semester.get().getSchool().equals(userRepo.findById(uid).get().getSchool())) {
            Semester semester = op_semester.get();
            List<Timeslot> timeslots = new ArrayList<>(semester.getTimeslots());
            timeslots.sort(Timeslot::compareTo); // Упорядочивание слотов по времени
            ...
            for (int i = 0; i < timeslots.size(); i++) {
                ...
                timeslotRepo.save(timeslots.get(i)); // Сохраняем изменения в базе данных
            }

            return new RedirectView("/logged-in/" + uid + "/semester/" + semester.getId() + "/calendar");
        } else {
            return new RedirectView("/err");
        }
    }
    ...
    /**
     * Парсит строку времени в формате "hh:mm" и возвращает массив из [часы, минуты].
     */
    ...
    /**
     * Преобразует часы и минуты в общее количество минут с полуночи.
     */
    ...
    /**
     * Проверяет, пересекаются ли два временных слота.
     */
    private boolean isTimeOverlapping(String start1, String end1, String start2, String end2) {
        int[] start1Parsed = parseTime(start1);
        int[] end1Parsed = parseTime(end1);
        int[] start2Parsed = parseTime(start2);
        int[] end2Parsed = parseTime(end2);

        int start1Minutes = convertToMinutes(start1Parsed[0], start1Parsed[1]);
        int end1Minutes = convertToMinutes(end1Parsed[0], end1Parsed[1]);
        int start2Minutes = convertToMinutes(start2Parsed[0], start2Parsed[1]);
        int end2Minutes = convertToMinutes(end2Parsed[0], end2Parsed[1]);

        // Слот 2 начинается до окончания слота 1
    ...
    public RedirectView add_to_group(@PathVariable Long uid, @PathVariable Long gid,
                                     @RequestParam String[] students, Model model,
                                     HttpServletRequest request) {
        RedirectView redirectView = new RedirectView();
        String checkAuthResult = checkAuth(uid, request);
        if (checkAuthResult.equals("**success**")) {
